export function encodeRpcEnvelope(message) {
  let bb = popByteBuffer();
  _encodeRpcEnvelope(message, bb);
  return toUint8Array(bb);
}

function _encodeRpcEnvelope(message, bb) {
  ...
  let $isRequest = message.isRequest;
  if ($isRequest !== undefined) {
    writeVarint32(bb, 16);
    writeByte(bb, $isRequest ? 1 : 0);
  }
...
export function decodeRpcEnvelope(binary) {
  return _decodeRpcEnvelope(wrapByteBuffer(binary));
}

function _decodeRpcEnvelope(bb) {
  let message = {};
